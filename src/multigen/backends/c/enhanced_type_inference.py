...
Adapted from CGen's EnhancedTypeInferenceEngine for MultiGen architecture.
"""
...
class EnhancedTypeInferenceEngine:
    """Advanced type inference engine for C backend STC containers."""
        ...
        class AssignmentAnalyzer(ast.NodeVisitor):
            ...
            def _infer_from_value(self, value_node: ast.expr, line_number: int) -> Optional[InferredType]:
                ...
                if isinstance(value_node, ast.List):
                    ...
                    # Filter out None keys (from dict unpacking like {**other_dict})
                    non_null_keys = [k for k in value_node.keys if k is not None]
                    key_type, value_type = self._infer_dict_types(non_null_keys, value_node.values)
                    ...
                    if isinstance(value_node.value, str):
                        return InferredType(
                            python_type="str",
                            c_type="char*",  # MultiGen uses char* not cstr
                            confidence=TypeConfidence.VERY_HIGH.value,
        ...
        class OperationAnalyzer(ast.NodeVisitor):
            ...
            def _infer_from_method_usage(
                ...
                if method_name in list_methods:
                    ...
                    return InferredType(
                        python_type="list",
                        c_type="vec_int",  # Default, will be refined
                    ...
                    return InferredType(
                        python_type="dict",
                        c_type="map_str_int",  # Default, will be refined
                        confidence=confidence,
                        source=f"method_usage_{method_name}",
                        line_number=line_number,
                    )
                    ...
                    return InferredType(
                        python_type="set",
                        c_type="set_int",  # Default, will be refined
                        confidence=confidence,
                        source=f"method_usage_{method_name}",
                        line_number=line_number,
                    )
                    ...
                    return InferredType(
                        python_type="str",
                        c_type="char*",  # MultiGen uses char*
                        confidence=TypeConfidence.HIGH.value,
                        source=f"method_usage_{method_name}",
                        line_number=line_number,
                    )
    ...
    def _map_python_to_c_type(self, python_type: str) -> str:
        """Map Python type to appropriate C type for MultiGen."""
        # Basic type mappings for MultiGen
        basic_mappings = {"int": "int", "float": "double", "str": "char*", "bool": "bool"}
        ...
        if python_type.startswith(("list[", "List[")):
            ...
            return "vec_int"  # Use actual STC type, not placeholder
        elif python_type in ["dict"]:
            return "map_str_int"  # Use actual STC type
        elif python_type in ["set"]:
            return "set_int"  # Use actual STC type
