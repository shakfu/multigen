...
and f-strings to C code using the mgen runtime library.
"""
...
class CStringMethodConverter:
    ...
    def convert_string_method(self, obj: str, method_name: str, args: list[str]) -> str:
        ...
        Returns:
            C code that calls the appropriate mgen runtime function
        ...
        if method_name == "upper":
            ...
            if len(args) == 0:
                return f"mgen_str_lstrip({obj})"
            elif len(args) == 1:
                return f"mgen_str_lstrip_chars({obj}, {args[0]})"
            else:
                raise UnsupportedFeatureError("str.lstrip() takes at most one argument")

        elif method_name == "rstrip":
            if len(args) == 0:
                return f"mgen_str_rstrip({obj})"
            elif len(args) == 1:
                return f"mgen_str_rstrip_chars({obj}, {args[0]})"
            else:
...
class CFStringConverter:
    ...
    using mgen_sprintf_string and related runtime functions.
    """
    ...
    def convert_f_string(self, expr: ast.JoinedStr) -> str:
        ...
        Example:
            f"Result: {x}" -> mgen_sprintf_string("Result: %s", mgen_int_to_string(x))
            f"Count: {len(items)}" -> mgen_sprintf_string("Count: %s", ...)
